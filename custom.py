...
def build_custom():
    ...
    inp,schart = st.columns(2,gap='large')
    with inp:
    ...
    if st.session_state.assets_data.shape[0] != 0:
        with schart:
            ...
            asset_info = df[df['Ticker'].isin(st.session_state.assets_data['Asset'])]
            fig = px.scatter(asset_info, x='IV', y='Total Return', hover_data='Ticker')
            st.plotly_chart(fig)
